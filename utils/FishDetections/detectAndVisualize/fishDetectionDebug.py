...
def detect_objects_on_tiles(tiles, model, output_dir, save_intermediate_outputs=True):
    all_boxes = []
    all_scores = []
    all_classes = []
    for tile_index, (tile, position) in enumerate(tiles):
        # Save the tile temporarily (required for your model's input format)
        temp_tile_path = os.path.join(output_dir, 'temp_tile.jpg')
        success = cv2.imwrite(temp_tile_path, tile)
        if not success:
            print(f"Failed to write {temp_tile_path}")
            continue  # Skip this tile if it fails to save
        ...
        # Draw boxes on tile and save, if requested
        if save_intermediate_outputs:
            fig, ax = draw_boxes_on_tile(tile, all_boxes, all_classes, class_labels, colors)
            plt.savefig(os.path.join(output_dir, f'tile_with_detections_{tile_index}.png'), dpi=75)
            plt.close()
        ...
        try:
            ...
            print(f"Error removing file {temp_tile_path}: {e}")

    # Convert boxes for NMS
    boxes_for_nms = [[x, y, x2 - x1, y2 - y1] for [x, y, x2, y2] in all_boxes]

    # Apply NMS
    indices = cv2.dnn.NMSBoxes(boxes_for_nms, all_scores, score_threshold=0.4, nms_threshold=0.99)
    ...
    for index in indices:
        ...
        i = index if np.isscalar(index) else index[0]  # Extract the actual index value

        # Append the unique boxes and classes
        unique_boxes.append(all_boxes[i])
        unique_classes.append(all_classes[i])

    return unique_boxes, unique_classes

def draw_boxes_on_tile(tile, boxes, classes, class_labels, colors):
    """
    Draw bounding boxes on a tile.
    """
...
def process_video_and_save_detections(video_path, model_path, output_folder):
    ...
    with open(detections_file, 'a') as file:  # Open in append mode
        for i in range(frame_count):
            ret, frame = cap.read()
            if not ret:
                break
            # Pass the new parameter to detect_objects_on_tiles
            tiles = tile_image(frame)
            boxes, classes = detect_objects_on_tiles(tiles, model, output_dir)

            # Write detections for the current frame
            frame_data = {'frame': i, 'boxes': boxes, 'classes': classes}
...
process_video_and_save_detections(video_path, model_path, output_folder)
